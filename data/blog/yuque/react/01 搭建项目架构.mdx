...
- **eslint**
  - `pnpm i -D -w eslint @typescript-eslint/parser`
  - `npx eslint --init`
- **husky**
  - `pnpm i -D -w husky`
  - `npx husky install`
...
- **commit**
  ...
  - _add .commitlintrc.json_
  - `npx husky add .husky/commit-msg 'npx --no-install commitlint -e $HUSKY_GIT_PARAMS'`
  - ![](https://cdn.jsdelivr.net/gh/zangguojun/tuchuang/yuque/Foc1vxRWDwr7HfszeXRDyIOBrySR.png)

### 2、配置 tsconfig.json

```json
{
  // 剔除: 声明是否需要在保存时候自动触发 tsc 编译的字段
  "compileOnSave": true,
  "compilerOptions": {
    "target": "ESNext",
    "module": "ESNext",
    "lib": ["ESNext", "DOM"],
    // 剔除: class 声明中的字段语义从 [[Set]] 变更到 [[Define]]
    ...
    // 剔除: 设置是否输出 js 文件，一般是设置为 false，将打包等工作交给 Webpack 等工具。
    "noEmit": true,
    // 有未使用的变量时，抛出错误
